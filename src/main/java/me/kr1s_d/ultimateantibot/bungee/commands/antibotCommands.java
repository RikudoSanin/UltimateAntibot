...
public class antibotCommands extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if(args.length == 0){
            sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getArgs_error())));
            return;
        }
        switch (args[0].toLowerCase()){
            ...
            case "clearwhitelist":
                if(sender.hasPermission("ab.clear") || sender.hasPermission("ab.admin")){
                    ...
                    sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getCommands_cleared("Whitelist"))));
                }
                return;
            case "clearblacklist":
                if(sender.hasPermission("ab.clear") || sender.hasPermission("ab.admin")){
                    antibotManager.getBlacklist().clear();
                    sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getCommands_cleared("Blacklist"))));
                }
                return;
            case "addwhitelist":
                if(sender.hasPermission("ab.addwhitelist") || sender.hasPermission("ab.admin")){
                    antibotManager.addWhitelist("/" + args[1]);
                    sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getCommands_added(args[1], "Whitelist"))));
                }
                return;
            case "addblacklist":
                if(sender.hasPermission("ab.addblacklist") || sender.hasPermission("ab.admin")){
                    antibotManager.addBlackList("/" + args[1]);
                    sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getCommands_added(args[1], "Blacklist"))));
                }
                return;
            case "removewhitelist":
                if(sender.hasPermission("ab.removewhitelist") || sender.hasPermission("ab.admin")){
                    antibotManager.removeWhitelist("/" + args[1]);
                    sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getCommands_removed(args[1], "Whitelist"))));
                }
                return;
            case "removeblacklist":
                if(sender.hasPermission("ab.removeblacklist") || sender.hasPermission("ab.admin")){
                    antibotManager.removeBlackList("/" + args[1]);
                    sender.sendMessage(new TextComponent(Utils.colora(Utils.prefix() + MessageManager.getCommands_removed(args[1], "BlackList"))));
                }
                return;
            case "stats":
